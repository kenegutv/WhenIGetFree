...
public class HUD : MonoBehaviour {
	...
	void DisplayBlueTroops(){
		GUI.Box (new Rect (Screen.width / 2 - 100, levelInfoHeight, 80, 25), "Blues: " + GameObject.FindGameObjectsWithTag("Blue").Length);
	}
	void DisplayRedTroops(){
		if(lM.typeOfLevel == LevelManager.LevelType.Battleground)
			GUI.Box (new Rect (Screen.width / 2 + 20, levelInfoHeight, 80, 25), "Reds: " + GameObject.FindGameObjectsWithTag("Red").Length);
		else if(lM.typeOfLevel == LevelManager.LevelType.Stealth)
	...
	void DisplayHP(){
		...
		GUI.Box (new Rect((hpBarPosX + 5), (hpBarPosY + 5), hpFillBarLength, Screen.height/20 - 10), ent.currentHealth + "/" + ent.maxHealth);
	}

	void DisplayAmmo(){

		GUI.Box (new Rect (ammoBoxPosX - 10, ammoBoxPosY - ammoBoxHeight + 2, ammoBoxWidth + 20, ammoBoxHeight - 6), "Ammo");
